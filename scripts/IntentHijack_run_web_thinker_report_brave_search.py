...
async def main_async():
    ...
    print(f"Setting concurrent limit to {args.concurrent_limit}")
    ...
    try:
        ...
        with tqdm(total=len(tasks)) as pbar:
            async def track_progress(task, idx):
                result = await task
                save_markdown(result, idx)  # Save markdown file in real-time
                pbar.update(1)
                return result
...
def main():
    print("Start job at ", time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
