    ...
    try:
        if use_jina:
            ...
            try:
                ...
                # Add encoding detection and processing
    ...
    async def acquire(self):
        ...
        async with self.lock:
            while self.tokens <= 0:
                ...
                if self.tokens <= 0:
                    await asyncio.sleep(random.randint(5, 30))  # Wait for random seconds before retry
            
            self.tokens -= 1
            return True

# Create global rate limiter instance
jina_rate_limiter = RateLimiter(rate_limit=130)  # 130 requests per minute to avoid errors
...
async def fetch_page_content_async(urls: List[str], use_jina: bool = False, jina_api_key: Optional[str] = None, 
    ...
    async def process_urls():
        ...
        async with aiohttp.ClientSession(connector=connector, timeout=timeout, headers=headers) as session:
            ...
            return {url: result for url, result in zip(urls, results)}  # Return dictionary instead of coroutine object

    return await process_urls()  # Ensure async operations complete
